...
async getTutorials(): Promise<Tutorial[]> {
  try {
    return await db
      ...
      .orderBy(
        asc(tutorials.category ?? ''),              // Group by category
        asc(tutorials.category_order ?? 999),        // Sort categories by custom order
        asc(tutorials.order ?? 9999)                 // Sort tutorials inside category
      );
  } catch (error) {
